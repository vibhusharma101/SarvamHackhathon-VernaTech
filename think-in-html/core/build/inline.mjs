...
export function validateAnalysis(analysis) {
  ...
  // Optional `design` (bespoke theme). The renderer sanitizes its contents at
  // runtime; here we only enforce the shape so a malformed theme fails loudly.
  if ('design' in analysis && analysis.design != null) {
    const d = analysis.design;
    if (typeof d !== 'object' || Array.isArray(d)) {
      throw new Error('Schema validation failed: "design" must be an object.');
    }
    if ('vars' in d && d.vars != null && (typeof d.vars !== 'object' || Array.isArray(d.vars))) {
      throw new Error('Schema validation failed: "design.vars" must be an object of CSS custom properties.');
    }
    if ('css' in d && d.css != null && typeof d.css !== 'string') {
      throw new Error('Schema validation failed: "design.css" must be a string.');
    }
  }
}
