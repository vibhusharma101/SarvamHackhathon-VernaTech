(function () {
  ...
  // ── Design sanitizers ──
  // The `design` object is untrusted data. These keep AI-authored theming from
  // introducing markup, JS-in-CSS, or external network fetches. Defense in depth
  // with the page's CSP (default-src 'none'; img-src data:), which already blocks
  // remote url()/@import at the browser level.
  var EXTERNAL_URL = /url\(\s*['"]?\s*(?!data:|#)/i;   // any url() not pointing at a data: URI or in-page fragment
  var CSS_HAZARD = /@import|expression\s*\(|javascript:|<\/?(?:style|script)/i;
  function okVarName(k) { return /^--[a-zA-Z0-9-]+$/.test(k); }
  function okVarValue(v) {
    if (typeof v !== 'string') return false;
    if (/[{};<>]/.test(v)) return false;                 // can't break out of the rule / inject markup
    if (CSS_HAZARD.test(v) || EXTERNAL_URL.test(v)) return false;
    return true;
  }
  function safeClass(c) { return String(c == null ? '' : c).replace(/[^A-Za-z0-9 _-]/g, '').trim(); }
  function safeSwatch(v) { return okVarValue(v) ? String(v).replace(/["']/g, '') : ''; } // goes into an inline style="" attribute
  function safeCss(css) {
    if (typeof css !== 'string') return '';
    return css
      .replace(/@import[^;]*;?/gi, '')                    // no remote stylesheets
      .replace(/url\(\s*['"]?\s*(?!data:|#)[^)]*\)/gi, 'none') // neutralize external url() fetches
      .replace(/expression\s*\(/gi, '(')                 // legacy IE JS-in-CSS
      .replace(/javascript:/gi, '')
      .replace(/<\/(style|script)/gi, '<\\/$1');         // defensive: never re-close the tag
  }
  var design = (data.design && typeof data.design === 'object') ? data.design : null;

  // The AI-authored theme becomes the default "Custom" skin; the 4 presets stay
  // available in the picker. Its swatch comes from the theme's own accent color.
  if (design) {
    var dv = (design.vars && typeof design.vars === 'object') ? design.vars : {};
    SKINS.unshift({ id: 'custom', label: safeClass(design.name) || 'Custom', sw: safeSwatch(dv['--grad-hot']) || safeSwatch(dv['--bg']) || 'linear-gradient(135deg,#7c3aed,#ec4899)' });
    state.skin = 'custom';
  }

  // Build the custom theme's CSS and inject it via textContent (never innerHTML).
  // Vars land on :root[data-skin="custom"] so switching to a preset skin cleanly
  // overrides them, and switching back restores the custom look.
  function injectDesign() {
    if (!design) return;
    var rule = ':root[data-skin="custom"]{';
    var dvars = (design.vars && typeof design.vars === 'object') ? design.vars : {};
    Object.keys(dvars).forEach(function (k) { if (okVarName(k) && okVarValue(dvars[k])) rule += k + ':' + dvars[k] + ';'; });
    rule += '}\n' + safeCss(design.css);
    var st = document.getElementById('tih-design') || document.createElement('style');
    st.id = 'tih-design';
    st.textContent = rule;
  ...
  function render() {
    ...
    (data.blocks || []).forEach(function (b, i) {
      var fn = R[b.type];
      if (!fn) return;
      var cls = 'tih-block tih-' + safeClass(b.type) + (b.className ? ' ' + safeClass(b.className) : '');
      h += '<div class="' + cls + '">' + fn(b, i) + '</div>';
    });
    h += '</div>' + footer();
  ...
  function bindEvents() {
    ...
    app.querySelectorAll('[data-scroll="next"]').forEach(function (el) {
      el.addEventListener('click', function () {
        ...
        var wrap = hook && hook.closest ? hook.closest('.tih-block') : null;
        var next = wrap ? wrap.nextElementSibling : (hook ? hook.nextElementSibling : null);
